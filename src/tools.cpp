...
/**
 * Parse scp1310jp1 address in a string form into
...
/**
 * Return string representation of scp1310jp1 address
 */
string
...
{
    // default path to scp1310jp1 folder
    // on linux this is /home/<username>/.bitscp1310jp1
    string default_scp1310jp1_dir = tools::get_default_data_dir();

    if (nettype == cryptonote::network_type::TESTNET)
        default_scp1310jp1_dir += "/testnet";
    if (nettype == cryptonote::network_type::STAGENET)
        default_scp1310jp1_dir += "/stagenet";


    // the default folder of the lmdb blockchain database
    // is therefore as follows
    return default_scp1310jp1_dir + string("/lmdb");
}
...
{
    // the default folder of the lmdb blockchain database
    string default_lmdb_dir   = sineg::get_default_lmdb_folder(nettype);
    ...
    blockchain_path = sineg::remove_trailing_path_separator(blockchain_path);
        ...
        vector<txin_to_key>& input_key_imgs)
{
    ...
    uint64_t sin_outputs       {0};
    uint64_t sin_inputs        {0};
    uint64_t mixin_no          {0};
    ...
    for (size_t i = 0; i < input_no; ++i)
    {
        ...
        sin_inputs += tx_in_to_key.amount;
    ...
    return {sin_outputs, sin_inputs, mixin_no, num_nonrct_inputs};
};
...
summary_of_in_out_rct(const json& _json)
{
    ...
    {
        sin_outputs += vout["amount"].get<uint64_t>();
    }
    ...
    return {sin_outputs, sin_inputs, no_outputs, no_inputs, mixin_no, num_nonrct_inputs};
};
...
{
    pair<uint64_t, uint64_t> sum_sin {0, 0};
    ...
    {
        sum_sin.first += vout["amount"].get<uint64_t>();
        ++sum_sin.second;
    }
...
sum_money_in_inputs(const string& json_str)
{
    ...
    for (json& vin: j["vin"])
    {
        sum_sin.first += vin["key"]["amount"].get<uint64_t>();
        ++sum_sin.second;
    }
...
{
    array<uint64_t, 2> sum_sin;

    sum_sin[0] = sum_money_in_inputs(tx);
    sum_sin[1] = sum_money_in_outputs(tx);
...
string
sin_amount_to_str(const uint64_t& sin_amount, string format)
{
    return fmt::format("{:0.12f}", SIN_AMOUNT(sin_amount));
}
